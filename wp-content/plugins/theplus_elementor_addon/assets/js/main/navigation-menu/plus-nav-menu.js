...
function theplus_navmenu_hover(){
	...
	$(".plus-navigation-wrap .menu-hover .navbar-nav .dropdown").on('mouseenter',function() {
        ...
        let thisMenu = $(this); 
        var productList = thisMenu.find('.product-list.pt-plus-isotope'),
            productListLoop = thisMenu.find('.post-inner-loop'),
            layoutType = productList[0]?.dataset.layoutType;

        if ( productList.length > 0 ) {
            HoverResizelayout(productListLoop, layoutType, thisMenu);
        }
    ...
    function HoverResizelayout(productListLoop, layoutType, thisMenu) {
		if( 'grid' === layoutType || 'masonry' === layoutType || 'fitRows' === layoutType){
		    setTimeout(function(){
				jQuery(productListLoop).isotope('reloadItems').isotope();
				productListLoop[0].closest('.dropdown-menu').style.height = 'unset';
		    }, 1000);
        } else if ('metro' === layoutType){
            setTimeout(function(){
	...
	var $=jQuery;
		$('.plus-navigation-wrap .menu-click .plus-navigation-menu .navbar-nav li.menu-item-has-children > a,.plus-navigation-wrap .menu-click .plus-navigation-menu .plus-mobile-menu.plus-menu-off-canvas .navbar-nav li.menu-item-has-children > a').on('click', function (event) {
			...
			if($(this).closest(".plus-navigation-inner.menu-click")){
                ...
                var productList = navSideItem.find('.product-list.pt-plus-isotope'),
                    productListLoop = navSideItem.find('.post-inner-loop'),
                    layoutType = productList[0]?.dataset.layoutType;

                if ( productList.length > 0 ) {
                    ClickResizelayout(productListLoop, layoutType, navSideItemSub);
                }
        ...
        function ClickResizelayout(productListLoop, layoutType, navSideItemSub) {
            if( 'grid' === layoutType || 'masonry' === layoutType || 'fitRows' === layoutType){
                setTimeout(function(){	
                    jQuery(productListLoop).isotope('reloadItems').isotope();
                }, 10);
